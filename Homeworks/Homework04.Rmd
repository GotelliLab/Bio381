...
1. Using the [Excel template](https://gotellilab.github.io/Bio381/Scripts/RScripts.html) we discussed in class, create (or modify) a data set with real metadata and real numbers from your own research.

2. Save from `.xlsx` to `.csv` format.

3. Open your new `.csv` file in RStudio or in a text editor and clean up the junk characters that were added in with Excel.

4. Build a webpage with R chunks (leave the default settings so the chunks are evaluated and your code is shown) in which you read in the data using `read.table`.

5. Use the functions `str()`, `summary()`, `table()`, and `head()` and `tail()` as we did in class to check your data for any errors. `head()` and `tail()` are new, but read about them by searching on them in th help window of Rstudio, or (equivalently) try `help(head)` from the console.
