...
- use in stochastic simulation modeling
- use by empiricists to generate artificial data

Random variable $X_i$ that can take on values for $i=1\dots \infty$, each with probability $p_i$.

The expectation is a weighted average, where the weights are the probablities:

$$E(X_i) = \sum_{i=1}^\infty p_i X_i$$

$$E(\sigma^2X_i) = E(\sum_{i=1}^\infty (X_i - E(X_i))^2$$


These are first and second "moments", usually interpreted as the mean and the variance. The third moment is the skewness (asymmetry left or right), and the fourth moment is kurtosis (how fat or skinny tailed the distribution is. 
...
## Create tiny histogram plotting function
```{r}
library(ggplot2)

# built in function is not so great
testData <- runif(1000)
qplot(testData)

# FUNCTION Histo
# input: xData = a numeric vector
# output: ggplot of histogram



Histo <- function(xData=runif(1000),fColor="tan") {
...
# FUNCTION iHisto
# input: xData = a numeric vector of integers
# output: ggplot of bar heights

iHisto <- function(xData=runif(1000),fColor="tan") {
...
Histo(testData)
testData <- rpois(n=1000,lambda=1.1)
iHisto(testData)
iHisto(xData=testData,fColor="goldenrod")
```
...
iHisto(xData=x,fColor="coral")

x <- rbinom(n=1000,size=40,p=0.0075)
iHisto(xData=x,fColor="darkgreen")
```
    
    
- Multinomial
  - a "balls in urns" analog of binomial
    * Range: [0, # of balls]
    * Parameters: n= number of random vectors to draw; size=number of balls to distribute; probs= probability weights for each of the different "urns" 
    * Interpretation: Distribution of number of balls in each of the (unequal) urns
    
```{r}
z <- rmultinom(n=1,size=20,prob=c(0.2,0.7,0.1))
print(z)

# equivalent with sample function:
